...
public class Day15Part2 {
	...
	private Boolean deadElf = true;

	public Integer execute() throws FileNotFoundException {

		Integer elvenAttackPower = 4;
		Integer result = 0;
		while(deadElf) {
			result = simulateWithElvenAttackPowerOf(elvenAttackPower);
			elvenAttackPower++;
		}
		return result;
	}

	private Integer simulateWithElvenAttackPowerOf(Integer elvenAttackPower) throws FileNotFoundException {
		deadElf = false;

		initializeMap(readInput("resources/day15/input.txt"), elvenAttackPower);
		Integer round = 0;
		while (true) {
	...
	private void initializeMap(ArrayList<String> inputList, Integer elvenAttackPower) {
		...
		for (String line : inputList) {
			for (int i = 0; i < line.length(); i++) {
				...
				if (Arrays.asList('G', 'E').contains(element)) {
					...
					if(element.equals('G'))
						playerList.add(new Player(new Position(currentColumn, currentLine), element, 3));
					else
						playerList.add(new Player(new Position(currentColumn, currentLine), element, elvenAttackPower));
				}
	...
	private class Player implements Comparable {
		...
		public Player(Position position, Character race, Integer attackPower) {
